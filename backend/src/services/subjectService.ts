...
export class SubjectService {
  ...
  async deleteSubject(id: string): Promise<ISubject | null> {
    ...
    const courses = await Course.find({ subject_id: subjectObjectId }).select('_id course_img');
    ...
    try {
      await session.withTransaction(async () => {
        ...
        imagesToDelete.push(...courses.map((c) => c.course_img).filter((v): v is string => Boolean(v)));
        ...
        if (moduleIds.length > 0) {
          ...
          await Promise.all([
            chapterIds.length > 0
              ? ChapterProgress.deleteMany({ chapter_id: { $in: chapterIds } }).session(session)
              : Promise.resolve(),
            questionIds.length > 0
              ? QuestionProgress.deleteMany({ question_id: { $in: questionIds } }).session(session)
              : Promise.resolve(),
            TestQuestion.deleteMany({ module_id: { $in: moduleIds } }).session(session),
            ShortAnswerQuestion.deleteMany({ module_id: { $in: moduleIds } }).session(session),
            FillInTheBlankQuestion.deleteMany({ module_id: { $in: moduleIds } }).session(session),
            Question.deleteMany({ module_id: { $in: moduleIds } }).session(session),
            Chapter.deleteMany({ module_id: { $in: moduleIds } }).session(session),
            Module.deleteMany({ course_id: { $in: courseIds } }).session(session),
          ]);
        }

        await Promise.all([
          CourseProgress.deleteMany({ course_id: { $in: courseIds } }).session(session),
          Course.deleteMany({ _id: { $in: courseIds } }).session(session),
        ]);

        // delete subject
        deletedSubject = await Subject.findByIdAndDelete(subjectObjectId).session(session);
      });
    } finally {
